...
impl CasUser {
    ...
    ///
    /// let cas_user = CasUser::new("user", None);
    /// assert_eq!(cas_user.into_raw(), "{\"username\":\"user\",\"attributes\":{}}");
    /// ```
    /// - With attributes:
    /// ```
    /// use cas_client_core::CasUser;
    ...
    /// attributes.insert("Attribute 2".to_string(), "value 2".to_string());
    /// let cas_user = CasUser::new("user", Some(attributes.clone()));
    /// assert_eq!(cas_user.into_raw(), serde_json::to_string(&cas_user).unwrap());
    /// ```
    ...
    /// attributes.insert("Attribute 2".to_string(), "value 2".to_string());
    /// let cas_user = CasUser::new("user", Some(attributes.clone()));
    /// assert_eq!(CasUser::from_raw(&cas_user.into_raw()), cas_user);
    /// ```
